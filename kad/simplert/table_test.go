...
import (
	...
	"github.com/libp2p/go-libdht/libp2p"
	"github.com/libp2p/go-libp2p/core/peer"

	kt "github.com/libp2p/go-libdht/internal/kadtest"
...
var (
	key0  = kad.NewKey256(zeroBytes(32))                          // 000000...000
	key1  = kad.NewKey256(append([]byte{0x40}, zeroBytes(31)...)) // 010000...000
	key2  = kad.NewKey256(append([]byte{0x80}, zeroBytes(31)...)) // 100000...000
	key3  = kad.NewKey256(append([]byte{0xc0}, zeroBytes(31)...)) // 110000...000
	key4  = kad.NewKey256(append([]byte{0xe0}, zeroBytes(31)...)) // 111000...000
	key5  = kad.NewKey256(append([]byte{0x60}, zeroBytes(31)...)) // 011000...000
	key6  = kad.NewKey256(append([]byte{0x70}, zeroBytes(31)...)) // 011100...000
	key7  = kad.NewKey256(append([]byte{0x18}, zeroBytes(31)...)) // 000110...000
	key8  = kad.NewKey256(append([]byte{0x14}, zeroBytes(31)...)) // 000101...000
	key9  = kad.NewKey256(append([]byte{0x10}, zeroBytes(31)...)) // 000100...000
	key10 = kad.NewKey256(append([]byte{0x20}, zeroBytes(31)...)) // 001000...000
	key11 = kad.NewKey256(append([]byte{0x30}, zeroBytes(31)...)) // 001100...100
)

func TestBasic(t *testing.T) {
	bucketSize := 100
	rt := New[kad.Key256](kt.NewID(key0), bucketSize)
...
func TestAddPeer(t *testing.T) {
	...
	rt := New[kad.Key256](kt.NewID(key0), 2)
...
func TestNearestPeers(t *testing.T) {
	...
	rt := SimpleRT[kad.Key256, libp2p.PeerID]{
		self:       key0,
		buckets:    make([][]peerInfo[kad.Key256, libp2p.PeerID], 0),
		bucketSize: bucketSize,
	}
	rt.buckets = append(rt.buckets, make([]peerInfo[kad.Key256, libp2p.PeerID], 0))
	...
	rt2 := SimpleRT[kad.Key256, libp2p.PeerID]{
		self:       key0,
		buckets:    make([][]peerInfo[kad.Key256, libp2p.PeerID], 0),
		bucketSize: bucketSize,
	}
	rt2.buckets = append(rt2.buckets, make([]peerInfo[kad.Key256, libp2p.PeerID], 0))

	rt2.buckets[0] = append(rt2.buckets[0], peerInfo[kad.Key256, libp2p.PeerID]{peerIds[1], key1})
	rt2.buckets[0] = append(rt2.buckets[0], peerInfo[kad.Key256, libp2p.PeerID]{peerIds[1], key1})
...
func TestTableConcurrentReadWrite(t *testing.T) {
	nodes := make([]*kt.ID[kad.Key32], 5000)
	for i := range nodes {
