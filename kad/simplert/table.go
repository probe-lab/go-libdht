...
import (
	...
	"github.com/libp2p/go-libdht/internal/kadtest"
...
var _ kad.RoutingTable[kad.Key256, kadtest.ID[kad.Key256]] = (*SimpleRT[kad.Key256, kadtest.ID[kad.Key256]])(nil)
...
func (rt *SimpleRT[K, N]) RemoveNode(n N) bool {
	return rt.RemoveKey(n.Key())
}
...
func (rt *SimpleRT[K, N]) NearestNodes(kadId K, n int) []N {
	...
	sort.SliceStable(peers, func(i, j int) bool {
		distI := peers[i].kadId.Distance(kadId)
		distJ := peers[j].kadId.Distance(kadId)
